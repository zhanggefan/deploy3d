...
def update_thread(vis_iter, vis):
    ...
    while not is_done:
        ...
        det_box3d = np.stack(track_data['boxes_3d'], axis=0)
        ...
        def render():
            ...
            if det_box3d is not None and len(det_box3d):
                det_box, det_vel, text_info = renderbox(det_box3d, det_names, track_ids)
                vis.remove_geometry('boxes')
                vis.add_geometry('boxes', det_box)
                
                if det_vel is not None:
                    vis.remove_geometry('vels')
                    vis.add_geometry('vels', det_vel)
